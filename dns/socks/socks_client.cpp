...
#define PRINT_EXCEPTION(e) std::printf("(%s) Exception: %s\n", __FUNCTION__, (e).what())
...
namespace socks
{
    ...
    asio::awaitable<bool> socks_base_client::connect(
        asio::ip::tcp::socket &socket, const std::string &remote_address, uint16_t remote_port)
    {
        ...
        if (proxy_type_ == proxy_type::none)
        {
            ...
            asio::ip::tcp::resolver::results_type endpoints = resolver.resolve(remote_address_, std::to_string(remote_port_));
        ...
        else if (proxy_type_ == proxy_type::socks4)
        {
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::unsupported_version));
        }
    ...
    void socks_base_client::set_auth(bool user_auth, const std::string &username, const std::string &password)
    {
    ...
    void socks_base_client::set_proxy(proxy_type type, const std::string &proxy_address, uint16_t proxy_port)
    {
        proxy_type_ = type;
    ...
    void socks_base_client::check_methods(uint8_t methods)
    {
        ...
        {
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::unsupported_authentication_method));
        }
    }
    ...
    void socks_base_client::check_reply(reply_status reply)
    {
        ...
        case reply_status::general_socks_server_failure:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::general_socks_server_failure));
            break;
        case reply_status::connection_not_allowed_by_ruleset:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::connection_not_allowed_by_ruleset));
            break;
        case reply_status::network_unreachable:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::network_unreachable));
            break;
        case reply_status::host_unreachable:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::host_unreachable));
            break;
        case reply_status::connection_refused:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::connection_refused));
            break;
        ...
        case reply_status::command_not_supported:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::command_not_supported));
            break;
        case reply_status::address_type_not_supported:
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::address_type_not_supported));
            break;
    ...
    asio::awaitable<bool> socks_base_client::connect_proxy(asio::ip::tcp::socket &socket)
    {
        asio::ip::tcp::resolver resolver(executor_);
        asio::ip::tcp::resolver::results_type endpoints = resolver.resolve(proxy_address_, std::to_string(proxy_port_));
    ...
    asio::awaitable<bool> socks_base_client::request_socks5(asio::ip::tcp::socket &socket)
    {
        // request auth
        ...
        try
        {
            co_await asio::async_write(socket, asio::buffer(socks_buffer_.data(), socks_buffer_.size()), asio::use_awaitable);

            // response auth
            socks_buffer_.resize(socks_buffer_size);
            co_await socket.async_read_some(asio::buffer(socks_buffer_.data(), socks_buffer_.size()), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            socket.close();
        ...
        {
            throw std::system_error(socks::errc::make_error_code(socks::errc::error_code::unsupported_version));
        }
        ...
        if (methods == 2)
        {
            ...
            {
                throw std::system_error(
                    socks::errc::make_error_code(socks::errc::error_code::unsupported_authentication_version));
            }

            if (status != 0)
            {
                throw std::system_error(socks::errc::make_error_code(socks::errc::error_code::authentication_error));
            }
        }
    ...
    asio::awaitable<bool> socks_base_client::request_command(asio::ip::tcp::socket &socket, uint8_t command)
    {
        // request auth
        uint8_t version = (uint8_t)proxy_type::socks5;
        uint8_t reserved = 0;
        ...
        std::printf("bind_address: %s:%d\n", bind_address_.c_str(), bind_port_);

        co_return true;
    }

    asio::awaitable<bool> socks_base_client::connect_socks5(asio::ip::tcp::socket &socket)
    {
    ...
    asio::awaitable<bool> socks_tcp_client::connect(const std::string &remote_address, uint16_t remote_port)
    {
        co_return co_await ::socks::socks_base_client::connect(socket_, remote_address, remote_port);
    }
    ...
    asio::awaitable<int> socks_tcp_client::write(const char *data, uint16_t data_length)
    {
        try
        {
            co_return co_await asio::async_write(socket_, asio::buffer(data, data_length), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            disconnect();
    ...
    asio::awaitable<int> socks_tcp_client::read(char *data, uint16_t data_length)
    {
        try
        {
            co_return co_await asio::async_read(socket_, asio::buffer(data, data_length), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            disconnect();
    ...
    asio::ip::tcp::socket &socks_tcp_client::get_socket()
    {
        return socket_;
    }

    bool socks_tcp_client::is_connected()
    {
        return socket_.is_open();
    }

    asio::awaitable<bool> socks_tcp_client::connect_socks5(asio::ip::tcp::socket &socket)
    {
    ...
    socks_tls_client::socks_tls_client(asio::any_io_executor executor, asio::ssl::context &ssl_context)
    ...
    asio::awaitable<int> socks_tls_client::write(const char *data, uint16_t data_length)
    {
        try
        {
            co_return co_await asio::async_write(tls_socket_, asio::buffer(data, data_length), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            disconnect();
    ...
    asio::awaitable<int> socks_tls_client::read(char *data, uint16_t data_length)
    {
        try
        {
            co_return co_await asio::async_read(tls_socket_, asio::buffer(data, data_length), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            disconnect();
    ...
    asio::awaitable<bool> socks_tls_client::connect_socks5(asio::ip::tcp::socket &socket)
    {
        bool status = co_await ::socks::socks_base_client::connect_socks5(socket);
    ...
    socks_udp_client::socks_udp_client(asio::any_io_executor executor)
        : socks_base_client(executor),
          socket_(executor),
          udp_socket_(executor),
          resolver_(executor)
    {
    ...
    asio::awaitable<bool> socks_udp_client::associate(const std::string &remote_address, uint16_t remote_port)
    {
        if (proxy_type_ == proxy_type::none)
        {
            remote_address_ = remote_address;
            ...
            endpoints_ = resolver_.resolve(remote_address_, std::to_string(remote_port_));
    ...
    asio::awaitable<int> socks_udp_client::send(const char *data, uint16_t data_length)
    {
        if (proxy_type_ == proxy_type::none)
        {
            ...
            try
            {
                length = co_await udp_socket_.async_send_to(asio::buffer(send_buffer_.data(), send_buffer_.size()),
        ...
        if (proxy_type_ == proxy_type::none)
        {
            ...
            try
            {
                length = co_await udp_socket_.async_receive(
                    asio::buffer(recv_buffer_.data(), recv_buffer_.size()), asio::use_awaitable);
            }
            catch (const std::system_error &e)
            {
                // ignore recv timeout
    ...
    asio::ip::udp::socket &socks_udp_client::get_socket()
    {
        return udp_socket_;
    }
    ...
    asio::awaitable<bool> socks_udp_client::connect_socks5(asio::ip::tcp::socket &socket)
    {
        bool status = co_await ::socks::socks_base_client::connect_socks5(socket);
        ...
        status = co_await request_command(socket, (uint8_t)proxy_command::udp_associate);
    ...
    asio::awaitable<int> socks_udp_client::send_socks(const char *data, uint16_t data_length)
    {
        uint16_t reserved = 0;
        uint8_t fragment = 0;
        uint8_t addr_type = (uint8_t)remote_type_;
        ...
        try
        {
            length = co_await udp_socket_.async_send_to(
                asio::buffer(send_buffer_.data(), send_buffer_.size()), *endpoints_.begin(), asio::use_awaitable);
        }
        catch (const std::system_error &e)
        {
            release();
    ...
    asio::awaitable<int> socks_udp_client::recv_socks(char *data, uint16_t data_length)
    {
        int length = 0;
        ...
        if (reserved != 0 || fragment != 0 || remote_type == address_type::none)
        {
            throw std::system_error(
                socks::errc::make_error_code(
                    socks::errc::error_code::udp_data_error));
        }
        ...
        {
            throw std::system_error(
                socks::errc::make_error_code(socks::errc::error_code::remote_data_error));
        }
        ...
        if (remote_type != remote_type_)
        {
            ...
            {
                if (remote_type == address_type::ipv4 || remote_type == address_type::ipv6)
                {
        ...
        co_spawn(
            ...
            [&]() -> asio::awaitable<void>
            {
                while (true)
                {
                    ...
                    {
                        socks_buffer_.resize(socks_buffer_size);
                        co_await socket_.async_read_some(asio::buffer(socks_buffer_.data(), socks_buffer_.size()), asio::use_awaitable);
                    }
                    catch (const std::system_error &e)
                    {
                        release();
