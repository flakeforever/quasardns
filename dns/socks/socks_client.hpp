...
namespace socks
{
    ...
    class socks_base_client : public std::enable_shared_from_this<socks_base_client>
    {
    public:
        socks_base_client(asio::any_io_executor executor);

        void set_auth(bool user_auth, const std::string &username, const std::string &password);
        void set_proxy(proxy_type type, const std::string &proxy_address, uint16_t proxy_port);
    ...
    protected:
        ...
        virtual asio::awaitable<bool> request_command(asio::ip::tcp::socket &socket, uint8_t command);
    ...
    public:
        ...
        asio::awaitable<bool> connect(const std::string &remote_address, uint16_t remote_port);
        void disconnect();
    ...
    {
    public:
        socks_tls_client(asio::any_io_executor executor, asio::ssl::context &ssl_context);
        ~socks_tls_client();
    ...
    public:
        ...
        asio::awaitable<bool> associate(const std::string &remote_address, uint16_t remote_port);
        void release();
